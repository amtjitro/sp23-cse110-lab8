describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadowRootButtonInnerText = await shadowRootButton.getProperty('innerText');
    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
    const buttonText = await shadowRootButtonInnerText.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountElemInnerText = await cartCountElem.getProperty('innerText');
    const innerText = await cartCountElemInnerText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      const shadowRoot = await productItems[i].getProperty('shadowRoot');
      const buttonElem = await shadowRoot.$('button');
      const innertext = await buttonElem.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    const cartItems = await page.$$('product-item');
    for (let i = 0 ; i < cartItems.length; i++) {
      const shadowRoot = await cartItems[i].getProperty('shadowRoot');
